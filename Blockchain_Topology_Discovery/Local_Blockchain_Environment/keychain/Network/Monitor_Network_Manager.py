...
class Network_Manager(Thread):
    ...
    def __init__(self, src_ip, src_port, src_service, network, bootstrap, blockchain_manager):
        ...
        self._topology_information = dict()
    ...
    def run(self):
        ...
        try:
            ...
            while not self.__stop_event.isSet():
                ...
                if (time.time() - start) > Network_Constant.TIMEOUT_RETRY_ASK_NEW_PEER:
    ...
    def treat_inv_packet(self, connection, inv_msg, inv_msg_size):
        ...
        for block_hash in interesting_block_hashes:
            ...
            self.add_topology_monitoring_information(connection, ("block", block_hash), time.time(), inv_msg_size)
        ...
        for tx_hash in interesting_tx_hashes:
            ...
            self.add_topology_monitoring_information(connection, ("tx", tx_hash), time.time(), inv_msg_size)
    ...
    def treat_tx_message(self, connection, tx_msg, tx_msg_size):
        ...
        if ("tx", transaction.__hash__()) in data_requested:
            ...
            self.add_topology_monitoring_information(connection, ("tx", transaction.__hash__()), time.time(),
                                                         tx_msg_size)
    ...
    def treat_block_message(self, connection, block_msg, block_msg_size):
        ...
        if ("block", block.getBlockHash()) in data_requested:
            ...
            self.add_topology_monitoring_information(connection, ("block", block.getBlockHash()),
                                                         time.time(), block_msg_size)
    ...
    def add_topology_monitoring_information(self, connection, info, timestamp, msg_size):
        #############################################################################
        #                           TOPOLOGY INFORMATIONS                           #
        #                                                                           #
        #  - For each broadcasted information, keep:                                #
        #       1. Node IP                                                          #
        #       2. Most recent sent time_to_crawl (received time_to_crawl - round_trip_time)          #
        #       (the earliest moment at which the peer received this information)   #
        #                                                                           #
        #############################################################################

        round_trip_time = self.get_round_trip_time(connection)
        node_ip = connection.get_node_info()[0]

        per_byte_rtt = round_trip_time/Ping_Message.Ping_Message(0).getMessageSize()

        sent_time = timestamp - (per_byte_rtt * msg_size) / 2

        if info in self._topology_information and node_ip not in self._topology_information[info]:
            self._topology_information[info][node_ip] = sent_time

        else:
            self._topology_information[info] = dict()
            self._topology_information[info][node_ip] = sent_time

    def get_topology_monitoring_information(self):
        return self._topology_information.copy()

    def remove_topology_monitoring_information(self, info_list):
        for info in info_list:
            if info in self._topology_information:
                del self._topology_information[info]
