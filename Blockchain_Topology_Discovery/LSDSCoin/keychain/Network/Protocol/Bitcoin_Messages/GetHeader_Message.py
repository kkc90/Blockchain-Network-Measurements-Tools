...
class GetHeader_Message(Bitcoin_Message):
    def __init__(self, version, block_locator_list, hash_stop):
    ...
    def getPayload(self):
        ...
        hash_stop_bytes = self._hash_stop.encode("utf-8")[0:32]
    ...
    def __eq__(self, other):
        return self._version == other.get_version() and self._nb_block_locator == other.get_nb_block_locator() \
               and self._block_locator_list == other.get_block_locator_list() and self._hash_stop == other.get_hash_stop()


def treat_getheader_packet(payload):
    version = int.from_bytes(payload[0:4], byteorder="little")
    block_locator_list = treat_block_locator_bytes(payload[4:-32])
    hash_stop = payload[-32::].decode("utf-8")

    return GetHeader_Message(version, block_locator_list, hash_stop)
