...
class Block_Message(Bitcoin_Message):
    ...
    def getPayload(self):
        ...
        prev_block_bytes = self._prev_block.encode("utf-8")

        merkle_root_bytes = self._merkle_root.encode("utf-8")
    ...
    def __eq__(self, other):
        return self._version == other.get_version() and self._prev_block == other.get_prev_block() \
...
def treat_block_packet(payload):
    ...
    prev_block = payload[4:36].decode("utf-8")

    merkle_root = payload[36:68].decode("utf-8")
    ...
    txn_count, txns_bytes = treat_variable_length_list(payload[80::])

    txn_list = list()

    i = 0
    while i < txn_count:
        tx_version, tx_in_list, tx_out_list, tx_witness_list, lock_time, txns_bytes = treat_tx_field(txns_bytes)

        tx = Tx_Message(tx_version, tx_in_list, tx_out_list, tx_witness_list, lock_time)

        txn_list.append(tx)

        i = i + 1

    return Block_Message(version, prev_block, merkle_root, timestamp, bits, nonce, txn_list)
