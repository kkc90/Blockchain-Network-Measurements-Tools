...
class Tx_Message(Bitcoin_Message):
    ...
    def getPayload(self):
        ...
        tx_in_bytes = tx_in_list_to_bytes(self._tx_in_list)
        tx_in_count_bytes = get_variable_length_integer(len(self._tx_in_list))

        tx_out_bytes = tx_out_list_to_bytes(self._tx_out_list)
        tx_out_count_bytes = get_variable_length_integer(len(self._tx_out_list))

        tx_witness_bytes = tx_witness_list_to_bytes(self._tx_witness_list)
...
def treat_tx_packet(payload):
    ...
    return Tx_Message(version, tx_in_list, tx_out_list, tx_witness_list, lock_time)
